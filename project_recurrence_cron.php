...
if ($result = mysqli_query($conn, $sql)) {
    if (mysqli_num_rows($result) > 0) {
        while ($pattern_details = mysqli_fetch_array($result)) {
         ...
         //    $recurDate = date('Y-m-d', strtotime('-'.$recur_days.' days', strtotime($dueDate)));
            $recurDate = $pattern_details['generation_date'];
            $curDate = date('Y-m-d');
            // if($pattern_details['project_id']==800){
            //     echo strtotime($curDate).'fff';
            //     echo strtotime($recurDate);
            // }
            if(strtotime($curDate)==strtotime($recurDate)){
             ...
             if($insert_project_recurrence_main_data['generation_month']==''){
                $insert_project_recurrence_main_data['generation_month'] = '0';
            }

            if($insert_project_recurrence_main_data['generation_day']==''){
                $insert_project_recurrence_main_data['generation_day'] = '0';
            }
            $generation_days = ((int)$insert_project_recurrence_main_data['generation_month']*30) + (int)$insert_project_recurrence_main_data['generation_day'];
            ...
            }elseif($insert_project_recurrence_main_data['pattern']=='annually'){
                $next_due_date = date("Y-m-d", strtotime("+1 year", strtotime($due_date)));
                $insert_project_recurrence_main_data['next_due_date'] = $next_due_date;
            }elseif($insert_project_recurrence_main_data['pattern']=='weekly'){
                $next_due_date = date("Y-m-d", strtotime("+7 days", strtotime($due_date)));
                $insert_project_recurrence_main_data['next_due_date'] = $next_due_date;
            }elseif($insert_project_recurrence_main_data['pattern']=='quarterly'){
