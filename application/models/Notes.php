...
//            echo $this->db->last_query();die;
        ...
        foreach ($notes_data as $note) {
            if (trim($note) != "") {
                ...
                if ($related_table_id == 2) {
                    ...
                    $this->db->from('action_staffs');
                    $this->db->where('action_id', $foreign_value);
                    $staffs_id = $this->db->get()->result_array();
                    foreach ($staffs_id as $sid) {
                        $this->db->insert("notes_log", ['note_id' => $note_id, 'user_id' => $sid['staff_id'], 'related_table_id' => $related_table_id, 'date_time' => date('Y-m-d H:i:s')]);
                    }
                }else{
    ...
    public function change_read_status_notes($id, $userid ='') {
        $this->load->model('action_model');
        $this->db->select('nl.user_id,an.id as note_id');
        $this->db->from('action_notes an');
        $this->db->join('notes_log nl','nl.note_id=an.id','innner');
        $this->db->where(['an.action_id'=>$id,'nl.related_table_id'=>2]);
        $action_note_details=$this->db->get()->result_array();
        if (!empty($action_note_details)) {
            foreach ($action_note_details as $key=>$val){
                if(in_array($userid, $val)){
                    $data = array(
                    'read_status' => '1'
                    );
                    $this->db->where('note_id',$val['note_id']);
                    $this->db->where('user_id',$val['user_id']);
                    $this->db->update('notes_log', $data);
                }
            }
        }
    }
