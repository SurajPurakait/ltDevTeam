...
//        $this->save_invoice_on_related_service($data['service_request_id']);
    ...
    public function refresh_report_dashboard_service() {
        $select = [
            'ord.id AS id',  
        ...
        $this->db->query('TRUNCATE report_dashboard_service');
        $this->db->query('SET SQL_BIG_SELECTS=1');
        $report_service_data = $this->db->query($query)->result_array();

        if (count($report_service_data) > 0) {
            foreach ($report_service_data as $rsd) {
                ...
                $sql_q_d_c_result = $this->db->query($sql_q_d_c)->row_array();
                
                $get_service_end_days_query = "select * from target_days where service_id='" . $rsd['services_id'] . "'";
                $get_service_end_days = $this->db->query($get_service_end_days_query)->row_array();
                ...
                $sql_q_d_c_a_result = $this->db->query($sql_q_d_c_a)->row_array();
