...
if (!empty($action_list)):
    foreach ($action_list as $key => $action):
                                    ...
                                    <?php if (isset($action_staffs[0]['staff_id']) && $action_staffs[0]['staff_id'] != 0) { ?>
                                        <td title="Assign To">
                                            ...
                                            if (count($action_staffs) != 1) {
                                                ...
                                                if ($action['department_id'] == 2) {
                                                    echo '<span class="text-info">' . get_office_info_by_id($action['office_id'])['office_id'] . '</span>';
                                                } else {
        ...
        <!--                                     <td title="Start Date"><?//= ($action["start_date"] != "0000-00-00") ? date("m/d/Y", strtotime($action["start_date"])) : "-"; ?></td>
                                    ...
                                    <?= '<td title="Files"><span id="actionfilespan' . $action["id"] . '">' . (($action["files"] > 0) ? '<a class="label label-success" href="javascript:void(0)" count="' . $action["files"] . '" id="actionfile' . $action["id"] . '" onclick="show_action_files(\'' . $action["id"] . '\')"><b>' . $action["files"] . '</b></a>' : '<a class="label label-warning" href="javascript:void(0)" count="' . $action["files"] . '" id="actionfile' . $action["id"] . '" onclick="show_action_files(\'' . $action["id"] . '\')"><b>' . $action["files"] . '</b></a>') . '</span></td>'; ?>
                                    <td title="Notes"><span> 
                                            <?php
