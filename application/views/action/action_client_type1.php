...
?>
    <div class="form-group client_type_div0">
        ...
        <div class="col-lg-10">
        <select class="form-control client_type_field0" name="office_id" id="staff_office" onchange="refresh_existing_action_client_list(this.value,'');" title="Office">
    ...
    <div class="form-group client_type_div0" id="client_list">
        <label class="col-lg-2 control-label">Client List</label>
        <div class="col-lg-10">
        <select class="form-control client_type_field0" name="client_list_id[]" id="client_list_ddl" title="Client List" <?php echo (isset($client_id) && $client_id !='') ? 'disabled' : ''; ?> onchange = "get_business_client_id();" multiple>
...
<script>
    ...
    <?php if(isset($action_id) && $action_id!=''){ ?>
        refresh_existing_action_client_list('<?= $office_id ?>','<?= $client_id ?>');
   <?php }else{ ?>
        refresh_existing_action_client_list('<?= $staff_info['office'] ?>','');
   <?php }?>
   $("#client_list_ddl").chosen();

   function refresh_existing_action_client_list(office_id = '', client_id = '') {
        $.ajax({
            type: "POST",
            data: {
                office_id: office_id,
            ...
            url: base_url + 'action/home/get_completed_orders_officewise',
            dataType: "html",
