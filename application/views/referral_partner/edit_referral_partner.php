            ...
            <div class="ibox float-e-margins">
                <div class="ibox-content">
                    <form class="form-horizontal" method="post" id="edit_lead_prospect">
                        ...
                        <div class="form-group lead-agent-class" <?php echo ($data['lead_source']==5) ? 'style="display:block;"' : 'style="display:none;"' ?>>
...
<!--                        <div class="form-group lead-client-class" <?php echo ($data['lead_source']==7) ? 'style="display:block;"' : 'style="display:none;"' ?>>
                            <label class="col-lg-2 control-label">Lead Client</label>
                            <div class="col-lg-10">
                                <input type="text" name="lead_client" id="lead_client" value="<?php if($data['lead_source']==7){
                                    echo $data['lead_agent']; }else{
                                        echo '';
                                    }?>" class="form-control">
                            </div>
                        </div>
                        <div class="form-group lead-client-class-other" <?php echo ($data['lead_source']==6) ? 'style="display:block;"' : 'style="display:none;"' ?>>
                            <label class="col-lg-2 control-label">Other</label>
                            <div class="col-lg-10">
                              <input type="text" name="lead_client_other" id="lead_client_other" value="<?php if($data['lead_source']==6){
                                    echo $data['lead_agent']; }else{
                                        echo '';
                                    }?>" class="form-control" >
                            </div>
...
<script>
    ...
    function changeLeadSource(lead_source_id) {
        if (parseInt(lead_source_id) == 7) {
            ...
            $(".lead-client-class-other").hide(); 
        }else if(Number(lead_source_id) == 6){
                $(".lead-client-class").hide();
                $(".lead-agent-class").hide();    
    ...
    function lead_source_box() {
        ...
        if (value == "Referral Agent") {
            $(".lead-agent-class").show();
            $(".lead-agent-class-other").hide();
        }else if (value == "Other") {
            $(".lead-agent-class-other").show();
            $(".lead-agent-class").hide();
        } else {
            $(".lead-agent-class").hide();
            $(".lead-agent-class-other").hide();
    ...
    function edit_referral_partner() {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.trim() == "1") {
                    swal({title: "Success!", text: "Referral Agent Successfully Updated!", type: "success"}, function () {
                        goURL(base_url + 'referral_partner/referral_partners/partners');
                    });
                } else if (result.trim() == "-1") {
                    swal("ERROR!", "Unable To Edit Referral Agent", "error");
                }
            },
            beforeSend: function () {
                openLoading();
