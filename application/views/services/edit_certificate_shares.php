    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <form class="form-horizontal" method="post" id="form_create_certificate_shares" onsubmit="request_create_certificate_shares(); return false;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10 related-service-div">
                                <label class="radio-inline">
                                    <input type="radio" name="service_id" id="certificate_shares_20" disabled="disabled" <?= $service_id == $twenty_certificate_shares['id'] ? 'checked' : ''; ?> onchange="loadDdlOptionsAjax('get_select_service', '', this.value);setIdVal('service_id', this.value);setIdVal('retail_price', '<?= $twenty_certificate_shares['retail_price']; ?>');$('.related_service').hide();" value="<?= $twenty_certificate_shares['id']; ?>">
                                    <label for="certificate_shares_20"><?= $twenty_certificate_shares['description']; ?></label>
                                </label><br>
                                <label class="radio-inline">
                                    <input type="radio" name="service_id" id="certificate_shares_cb" disabled="disabled" <?= $service_id == $corp_book['id'] ? 'checked' : ''; ?> onchange="loadDdlOptionsAjax('get_select_service', '', this.value);setIdVal('service_id', this.value);setIdVal('retail_price', '<?= $corp_book['retail_price']; ?>');$('.related_service').hide();" value="<?= $corp_book['id']; ?>">
                        ...
                        <div id="related_service_container">
                            ...
                            foreach ($all_related_services as $key => $data) {
                                if ($data['related_services_id'] != 1) {
                                    ...
                                    if (in_array($data['related_services_id'], $selected_services)) {
                                        $specific_services = getSpecificServiceData($edit_data['id'], $data['related_services_id']);
                                        ?>
                                        <div id="related_service_<?= $data['related_services_id'] ?>" data-serviceid="<?= $data['related_services_id'] ?>" class="related_service bg-<?= $key + 1 ?> row"  style="display: block">
                                        <?php } else { ?>
                                            <div id="related_service_<?= $data['related_services_id'] ?>" data-serviceid="<?= $data['related_services_id'] ?>" class="related_service bg-<?= $key + 1 ?> row"  style="display: none">
                                                <?php
                                                $specific_services = getSpecificServiceData($edit_data['id'], $data['related_services_id']);
                                            }
                                            ?>
                            ...
                            <?= service_note_func('Note', 'n', 'order', $edit_data['id']); ?>                            
                            <div class="hr-line-dashed"></div>
...
//    loadDdlOptionsAjax('get_select_service', [], '<?= $service_id; ?>');
    $(function () {
        $('.service-for-20, .service-for-cb').removeAttr('checked').hide();
