...
if (!function_exists('getTemplateCategoryProjectList')) {
    function getTemplateCategoryProjectList($template_id, $template_cat_id = '', $month = '', $year)
    {
        $ci = &get_instance();
        $ci->load->model("project_template_model");
        return $ci->project_template_model->get_project_list('', '', $template_id, '', '', '', '', '', '', '', '', '', $template_cat_id, $month, $year);
    }
}
...
if (!function_exists('get_office_name_for_action_view')) {
    function get_office_name_for_action_view($id)
    {
        $ci = &get_instance();
        $ci->load->model('action_model');
...
if (!function_exists('count_new_services')) {

    function count_new_services($status, $by = "", $category = "", $request_by = "")
    {
        $ci = &get_instance();
        $ci->load->model('service_model');
            ...
            if ($category == "") {
                ...
                if ($status == 2) {
                    ...
                    if ($ci->session->userdata('nts_services')) {
                        ...
                        return $_SESSION['nts_services'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1) {
                    ...
                    if ($ci->session->userdata('s_services')) {
                        ...
                        return $_SESSION['s_services'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                }
            } else {
                ...
                if ($status == 2 && $category == 1) {
                    if ($ci->session->userdata('nts_incorporation')) {
                        ...
                        return $_SESSION['nts_incorporation'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 2 && $category == 2) {
                    if ($ci->session->userdata('nts_accounting')) {
                        return $ci->session->userdata('nts_accounting');
                    } else {
                        return $_SESSION['nts_accounting'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 2 && $category == 3) {
                    if ($ci->session->userdata('nts_tax')) {
                        return $ci->session->userdata('nts_tax');
                    } else {
                        return $_SESSION['nts_tax'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 2 && $category == 4) {
                    if ($ci->session->userdata('nts_business')) {
                        return $ci->session->userdata('nts_business');
                    } else {
                        return $_SESSION['nts_business'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 2 && $category == 5) {
                    if ($ci->session->userdata('nts_partner')) {
                        return $ci->session->userdata('nts_partner');
                    } else {
                        return $_SESSION['nts_partner'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1 && $category == 1) {
                    if ($ci->session->userdata('s_incorporation')) {
                        return $ci->session->userdata('s_incorporation');
                    } else {
                        return $_SESSION['s_incorporation'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1 && $category == 2) {
                    if ($ci->session->userdata('s_accounting')) {
                        return $ci->session->userdata('s_accounting');
                    } else {
                        return $_SESSION['s_accounting'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1 && $category == 3) {
                    if ($ci->session->userdata('s_tax')) {
                        return $ci->session->userdata('s_tax');
                    } else {
                        return $_SESSION['s_tax'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1 && $category == 4) {
                    if ($ci->session->userdata('s_business')) {
                        return $ci->session->userdata('s_business');
                    } else {
                        return $_SESSION['s_business'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                } elseif ($status == 1 && $category == 5) {
                    if ($ci->session->userdata('s_partner')) {
                        return $ci->session->userdata('s_partner');
                    } else {
                        return $_SESSION['s_partner'] = $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
                    }
                }
            }
        } else {

            return $ci->service_model->count_new_service_filter($status, $by, $category, $request_by);
        }
    }
}
