    ...
    if ($reports_data_count > 0) {
        while ($rpd = mysqli_fetch_assoc($reports_data)) { 
            for($i=1; $i <= $rpd['services']; $i++) {
                ...
                $department_id = $service_detail['service_department'];
                $category_id = $service_detail['service_category'];
                $office_id = $rpd['office_id'];
                $office_id_name = $rpd['officeid'];
                $created_by = $rpd['created_by'];
                $service_status = $rpd['service_status'];
                ...
                $rqr = mysqli_fetch_assoc($royalty_query_run);
                    if ($invoice_id_inner == $rqr['service_id']) {
                        ...
                        if (empty(array_diff($comparison_array,$rqr))) {
                            ...
                            if($department_id != $rqr['department_id']) {
                                $update_sql .= "`department_id`='$department_id',";
                            }
                            if($category_id != $rqr['category_id']) {
                                $update_sql .= "`category_id`='$category_id',";
                            }
                        ...
                        $sql_query = "INSERT INTO `report_dashboard_service`(`service_name`, `status`,`order_date` ,`date_completed`, `date_complete_actual`, `late_status`, `sos`, `category`, `department`, `office`) VALUES ('$service_name','$status','$order_date','$date_completed', '$date_complete_actual', '$  late_status', '$sos', '$category', '$department', '$office')";
